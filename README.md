...
Many papers and documents refer to music visualization as a hard problem to solve, ML algorithms are using some kind of piano roll or midi as data source and encoding/decoding using piano based vectors, writing and reading music software has also some issues for mapping notes when using piano as writing machine, music has a steep learning rate, from playing the instrument to how to read/write from a stave. We think that some of this issues could be solved using this system which gathers better information from a stave than any other one.  
